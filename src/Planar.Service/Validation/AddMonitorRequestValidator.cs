...
namespace Planar.Service.Validation
{
    public class AddMonitorRequestValidator : AbstractValidator<AddMonitorRequest>
    {
        public AddMonitorRequestValidator(GroupData dal, JobKeyHelper jobKeyHelper)
        {
            ...
            ////RuleFor(r => r.JobKey).JobIdExists(jobKeyHelper).NotEmpty().When(r => string.IsNullOrEmpty(r.JobGroup)).WithMessage("'{PropertyName}' must have value if 'Job Group' is empty");
            ////RuleFor(r => r.JobGroup).NotEmpty().When(r => string.IsNullOrEmpty(r.JobKey)).WithMessage("{PropertyName} must have value if 'Job Id' is empty"); ;
