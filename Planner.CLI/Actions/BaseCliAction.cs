...
namespace Planner.CLI.Actions
{
    ...
    {
        public static IIpcClient<IPlannerCommand> Proxy { get; set; }

        protected static async Task<ActionResponse> ExecuteEntity(Expression<Func<IPlannerCommand, BaseResponse<string>>> exp)
        {
    ...
    public class BaseCliAction<T> : BaseCliAction
    {
        ...
        {
            using Stream stream = typeof(Program).Assembly.GetManifestResourceStream($"Planner.CLI.Help.{name}.txt");
